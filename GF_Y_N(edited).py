...
if os.path.exists('new.catalog.csv'): 
    begin = input("Do you want to start where you left off? enter 'restart' if you want to star over./n 'a' if you want to revist a YB: ")
    if begin == 'Restart' or begin == 'restart' or begin == 'RESTART':  
        h_catalog = pd.read_csv('vel_with_gf.csv')
        h_catalog["Good fit"] = ""
...
#get the YB's location and radius
    ...
    elif YB_long >=7 and YB_long < 9 and YB_long >= -0.5 and YB_long <= 0.5:
    ...
    if begin == 'a' or begin =='A':
        continue_ = 'y'
        while continue_ == 'y' or continue_ == 'Y': 
            YB_inpt = int(input("Enter the YB number you want to revist: "))
            gf_plt = mpimg.imread('TRIAL_amplitude is bounded and sd is 2/box_spec_%03i.png' % (YB_inpt))

            imgplot = plt.imshow(gf_plt)
            plt.show()
            ...
            continue_ = input("Would you like to coninue revisting? Y/N: ")
        if continue_ == 'n' or continue_ == 'N':
            begin = input("would you like to continue on the rest of the YB? Y/N: ")
            if begin == 'n' or begin == 'N':
                break
  
    
    if GF == "" or pd.isnull(GF):
    ...
    if GF == "" or pd.isnull(GF):
        answer = input("Enter 'y' if you would like to continue. 'a' if you would like to revist a YB. 'n' if you want to save and stop. : ")
